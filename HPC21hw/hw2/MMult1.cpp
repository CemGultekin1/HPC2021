...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
 ...
 for (long p = 0; p < k; p+=BLOCK_SIZE) {
    #pragma omp parallel for if(PARALLEL_FLAG>0)
    for (long j = 0; j < n; j+=BLOCK_SIZE) {
        for (long i = 0; i < m; i+=BLOCK_SIZE) {
            for (long jb = 0; jb < BLOCK_SIZE; jb++) {
              for (long pb = 0; pb < BLOCK_SIZE; pb++) {
                for (long ib = 0; ib < BLOCK_SIZE; ib++) {
                  double A_ip = a[i+ib+(p+pb)*m];
                  double B_pj = b[p+pb+(j+jb)*k];
                  double C_ij = c[i+ib+(j+jb)*m];
                  C_ij = C_ij + A_ip * B_pj;
                  c[i+ib+(j+jb)*m] = C_ij;
                }
              }
...
int main(int argc, char** argv) {
 const long PFIRST = BLOCK_SIZE;
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double bandwidth = 4*m*n*k*NREPEATS/time/1e9; // TODO: calculate from m, n, k, NREPEATS, time
